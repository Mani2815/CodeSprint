...
export async function TimelineSection() {
  const eventId = await getActiveEventId(ACTIVE_EVENT_SLUG).catch(() => null);
  const checkpoints = eventId ? await prisma.checkpoint.findMany({ where: { eventId }, orderBy: { order: 'asc' } }) : [];
            ...
            return (
              <StaggerItem key={step.label} className="relative z-10 flex flex-col">
                ...
                <div className="flex flex-1 flex-col rounded-2xl border border-border/50 bg-card p-6 shadow-sm transition-shadow hover:shadow-md">
                  <Eyebrow className="text-primary">{checkpoint?.label || step.label}</Eyebrow>
                  <H4 className="mt-2 text-lg">{step.title}</H4>
                  <Text className="mt-2 text-sm text-muted-foreground">{step.description}</Text>
                  {checkpoint?.startDate && checkpoint?.submissionCloseDate && (
                    <Text className="mt-4 text-xs font-medium text-primary bg-primary/10 w-fit px-2 py-1 rounded-md">
                      {new Date(checkpoint.startDate).toLocaleDateString()} – {new Date(checkpoint.submissionCloseDate).toLocaleDateString()}
                    </Text>
                  )}
                </div>
              </StaggerItem>
