      ...
      <div className="mx-auto max-w-5xl space-y-8 p-6 lg:p-8">
        <div>
          ...
          <Muted className="mt-2">
            Insights into your team's collaboration and codebase health.
          </Muted>
        </div>
        <GithubConnectForm />
      </div>
        ...
        {latestSubmission && (
          <div className="flex items-center gap-4">
            <Muted className="text-xs">
              Last synced:{' '}
              {latestSubmission.analytics?.syncedAt
                ? new Date(latestSubmission.analytics.syncedAt).toLocaleString()
                : 'Never'}
            </Muted>
            {latestSubmission.isDraft && <GithubRemoveButton />}
            <GithubSyncButton />
          </div>
        )}
      </div>
              ...
              <div className="space-y-6">
                {memberStats.map((member, index) => {
                  ...
                  return (
                    <div
                      key={member.id}
                      className="space-y-3 border-b border-border/50 pb-4 last:border-0 last:pb-0"
                    >
